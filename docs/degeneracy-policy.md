...
| `Unbounded` edge, endpoints with opposite-sign near-`f64::MAX` coordinates | `direction` is always finite and non-zero (hardening round, post-0.7.0-implementation): a plain `pv - pu` component difference can overflow to `Infinity` here, so `edge_vector` falls back to a fixed `2^-600`-rescaled difference when it does — never scaled back (`direction` has no magnitude contract) and, on the non-overflowing axis, can legitimately underflow a tiny true difference to exactly `0.0` (harmless: that axis is negligible next to the overflowing one regardless). The *side* (`orient2d`, on the original unscaled points) is unchanged and, at this magnitude, itself a pre-existing, out-of-scope limit — see the next row. |
...
| Coordinate magnitude mixing tiny and extreme values in one call (e.g. one point near `4e304`) | **Discovered, not fixed, by this hardening round's `fuzz/voronoi_geometry` target**: `orient2d` itself can return *permutation-inconsistent* answers for the same 3 points (`Clockwise` for `(a,b,c)`, `Collinear` for `(a,c,b)`) — not just reduced precision. Downstream, `delaunay2()` assumes antisymmetry holds and can panic (`index out of bounds`) on such input; unrelated to `Voronoi2`/ADR-009 and out of scope for this round — tracked in `tasks/todo.md`. |
| An internal topology invariant this crate's own construction never violates (empty `VoronoiVertexId` face group; an `Unbounded` edge's source Delaunay edge with no incident face) | `Err(VoronoiGeometryError::InvalidTopology)`, not a panic — checked rather than trusted, matching every other case on this page. Not reachable from a `Voronoi2` built by `voronoi2()`; confirmed by deliberately corrupting a `Voronoi2`/`Triangulation2` in `src/triangulation/voronoi.rs`'s own tests (the latter via a `#[cfg(test)]`-only corruption hook in `delaunay2.rs`, since `Triangulation2`'s fields are private to a sibling module). |
